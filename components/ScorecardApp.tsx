...
import {
  ...
  classifySegment, scoreColor, scoreLabel,
...
export default function ScorecardApp() {
  ...
  const [gateData, setGateData] = useState<GateData>({
    name: "", email: "", phone: "", role: "", company: "Draper Fence & Rail Co.",
  });
  ...
  const segment = classifySegment(scores, totalScore);
  const teaser = getTeaserText(scores, segment);

  const section = SECTIONS[currentSection];
  const sectionAnswered = section?.questions.every((q) => scores[q.id] !== undefined);

  const completedQs = SECTIONS.slice(0, currentSection).reduce((a, s) => a + s.questions.length, 0) +
  ...
  function validateGate() {
    ...
    if (!gateData.name.trim() || gateData.name.trim().length < 2) e.name = "Please enter your full name.";
    if (!isValidEmail(gateData.email)) e.email = "Please enter a valid business email address.";
    if (!gateData.role.trim()) e.role = "Please enter your role.";
    if (!isValidPhone(gateData.phone)) e.phone = "Please enter a valid 10-digit US phone number.";
    setGateErrors(e);
  ...
  async function handleSubmit() {
    ...
    try {
      await fetch("/api/submit", {
        ...
        body: JSON.stringify({ ...gateData, scores, totalScore, maxScore: MAX_SCORE, pct, segment: segment.primary, secondarySegment: segment.secondary }),
      });
    } catch (_) { /* always proceed */ }
    setSubmitting(false);
    setStep("results");
  }

  const fadeUp = { initial: { opacity: 0, y: 20 }, animate: { opacity: 1, y: 0 }, exit: { opacity: 0, y: -20 }, transition: { duration: 0.32 } };
  const slideIn = { initial: { opacity: 0, x: 30 }, animate: { opacity: 1, x: 0 }, exit: { opacity: 0, x: -30 }, transition: { duration: 0.26 } };
        ...
        {step === "intro" && (
          <motion.div key="intro" {...fadeUp} style={{ padding: "32px 20px" }}>
            ...
            <p style={{ color: "#888", fontSize: 15, lineHeight: 1.65, marginBottom: 24 }}>
              Answer {TOTAL_QUESTIONS} questions about your sales operation. Get your exact score, your segment, and a personalized roadmap — including what it means for your exit valuation.
            </p>

            {/* Stats */}
            <div className="section-card" style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 12, textAlign: "center", marginBottom: 20 }}>
              {[{ v: "3 min", l: "To complete" }, { v: String(TOTAL_QUESTIONS), l: "Questions" }, { v: "Free", l: "Assessment" }].map(s => (
                <div key={s.l}>
            ...
            <div className="section-card" style={{ marginBottom: 20 }}>
              <p style={{ color: "#666", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 12 }}>What you&apos;ll discover</p>
              {[
                "Your exact sales maturity score across 5 dimensions",
                "Which STG Pain-Qualified Segment your business falls into",
                "The specific gaps impacting your exit valuation right now",
            ...
            {/* Appointment note */}
            <div className="section-card" style={{ borderColor: "rgba(0,217,255,0.15)", background: "rgba(0,217,255,0.04)", marginBottom: 24 }}>
              <p style={{ fontSize: 13, color: "#999", lineHeight: 1.65 }}>
                📅 <strong style={{ color: "#fff" }}>You have an appointment set</strong> with our Senior Growth Consultant <strong style={{ color: "#00D9FF" }}>Rob</strong> at 11 AM to discuss next steps on the proposal. Complete this first — we&apos;ll have your full analysis ready to discuss. We&apos;d love to have Draper Fence &amp; Rail in our <strong style={{ color: "#00D9FF" }}>February Cohort</strong>.
              </p>
            </div>
          ...
          <motion.div key={`quiz-${currentSection}`} {...slideIn} style={{ padding: "24px 20px" }}>
            ...
            <div style={{ marginBottom: 24 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 8 }}>
                <span style={{ color: "#555", fontSize: 12 }}>Section {currentSection + 1} of {SECTIONS.length}</span>
                <span style={{ color: "#00D9FF", fontSize: 12, fontWeight: 600 }}>{progressPct}% complete</span>
              </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 28 }}>
              {section.questions.map((q, qi) => {
                ...
                return (
                  ...
                  >
                    <p style={{ fontWeight: 600, fontSize: 15, lineHeight: 1.4, color: "#fff", marginBottom: 4 }}>{q.text}</p>
                    {q.context && <p style={{ color: "#555", fontSize: 12, lineHeight: 1.5, marginBottom: 12 }}>{q.context}</p>}
                    ...
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(5,1fr)", gap: 8, marginBottom: 6 }}>
                      {[1, 2, 3, 4, 5].map((val) => {
                        ...
                        return (
                          <button key={val} onClick={() => handleScore(q.id, val)} style={{
                            ...
                            <span style={{ fontSize: 20, fontWeight: 900, color: on ? colors[val - 1] : "#444" }}>{val}</span>
                          </button>
                        );
                      })}
                    </div>
            ...
            <div style={{ display: "flex", gap: 12, marginTop: 36 }}>
              ...
              <button className="cta-btn" style={{ flex: 1 }} disabled={!sectionAnswered} onClick={handleNext}>
                {currentSection < SECTIONS.length - 1 ? `Next: ${SECTIONS[currentSection + 1].title} →` : "See My Results →"}
              </button>
            </div>
            {!sectionAnswered && (
        ...
        {step === "teaser" && (
          <motion.div key="teaser" {...fadeUp} style={{ padding: "32px 20px" }}>
            <div className="section-card" style={{ textAlign: "center", padding: 28, marginBottom: 20 }}>
              ...
              <div style={{ position: "relative", display: "inline-block", marginBottom: 16 }}>
                <svg width={120} height={120} viewBox="0 0 120 120">
                  ...
                  <circle cx={60} cy={60} r={40} fill="none" stroke={ringColor} strokeWidth={10}
                ...
                <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", filter: "blur(6px)" }}>
                ...
                <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
                  <div style={{ background: "rgba(0,0,0,0.75)", borderRadius: "50%", width: 42, height: 42, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20, backdropFilter: "blur(4px)" }}>🔒</div>
                </div>
              </div>
              ...
              <div style={{
                ...
                <p style={{ color: "#00D9FF", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 8 }}>⚡ Initial Signal</p>
            ...
            <div className="section-card" style={{ marginBottom: 24 }}>
              <p style={{ color: "#555", fontSize: 11, textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 14 }}>Inside your full report</p>
              {SECTIONS.map((s) => {
        ...
        {step === "gate" && (
          <motion.div key="gate" {...fadeUp} style={{ padding: "32px 20px" }}>
            ...
            <p style={{ color: "#777", fontSize: 14, lineHeight: 1.55, marginBottom: 28 }}>
              Your detailed scorecard, segment analysis, and personalized recommendations will be texted and emailed to you.
            </p>

            <div style={{ display: "flex", flexDirection: "column", gap: 18 }}>
              {/* Name */}
              <div>
                <label style={{ display: "block", color: "#666", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.7px", marginBottom: 8 }}>Full Name</label>
                <input className={`stg-input${gateErrors.name ? " error" : ""}`} type="text" placeholder="Homa Wahab-Draper"
                  value={gateData.name} onChange={(e) => setGateData(p => ({ ...p, name: e.target.value }))}
                  onFocus={() => setGateErrors(p => ({ ...p, name: "" }))} />
                {gateErrors.name && <p style={{ color: "#ef4444", fontSize: 12, marginTop: 4 }}>{gateErrors.name}</p>}
              </div>
              {/* Role */}
              <div>
                <label style={{ display: "block", color: "#666", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.7px", marginBottom: 8 }}>Your Role</label>
                <input className={`stg-input${gateErrors.role ? " error" : ""}`} type="text" placeholder="Owner / CEO"
                  value={gateData.role} onChange={(e) => setGateData(p => ({ ...p, role: e.target.value }))}
                  onFocus={() => setGateErrors(p => ({ ...p, role: "" }))} />
                {gateErrors.role && <p style={{ color: "#ef4444", fontSize: 12, marginTop: 4 }}>{gateErrors.role}</p>}
              </div>
              {/* Company */}
              <div>
                <label style={{ display: "block", color: "#666", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.7px", marginBottom: 8 }}>Company</label>
                <input className="stg-input" type="text" value={gateData.company}
                  onChange={(e) => setGateData(p => ({ ...p, company: e.target.value }))} />
              </div>
              {/* Email */}
              <div>
                <label style={{ display: "block", color: "#666", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.7px", marginBottom: 8 }}>Business Email</label>
                <input className={`stg-input${gateErrors.email ? " error" : ""}`} type="email"
                  placeholder="hdraper@draperconstruction.net" inputMode="email" autoComplete="email"
                  value={gateData.email} onChange={(e) => setGateData(p => ({ ...p, email: e.target.value }))}
                  onFocus={() => setGateErrors(p => ({ ...p, email: "" }))} />
                {gateErrors.email && <p style={{ color: "#ef4444", fontSize: 12, marginTop: 4 }}>{gateErrors.email}</p>}
              </div>
              {/* Phone */}
              <div>
                <label style={{ display: "block", color: "#666", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.7px", marginBottom: 4 }}>Contact Number</label>
                <p style={{ color: "#444", fontSize: 11, marginBottom: 8 }}>so we can text you your results</p>
                <input className={`stg-input phone-input${gateErrors.phone ? " error" : ""}`}
                  type="tel" placeholder="(317) 201-5732" inputMode="tel" autoComplete="tel"
        ...
        {step === "results" && (
          <motion.div key="results" {...fadeUp} style={{ padding: "32px 20px" }}>
            ...
            <div style={{ marginBottom: 24 }}>
              ...
              <h2 style={{ fontSize: 24, fontWeight: 900, lineHeight: 1.2, marginBottom: 4 }}>Your Sales Team Scorecard</h2>
            ...
            <div className="section-card" style={{ textAlign: "center", padding: 28, marginBottom: 20 }}>
              <div style={{ position: "relative", display: "inline-block", marginBottom: 16 }}>
                <svg width={140} height={140} viewBox="0 0 140 140">
                  ...
                  <circle cx={70} cy={70} r={55} fill="none" stroke={ringColor} strokeWidth={12}
              ...
              <div style={{ marginBottom: 6 }}>
                ...
                <span style={{ fontWeight: 800, fontSize: 18, color: segment.color, marginLeft: 8 }}>{segment.primary}</span>
              </div>
            ...
            {/* Section breakdown */}
            <div className="section-card" style={{ marginBottom: 16 }}>
              <p style={{ fontWeight: 700, fontSize: 11, color: "#555", textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 18 }}>Score Breakdown</p>
              {SECTIONS.map((s) => {
                const ss = getSectionScore(s);
                ...
                return (
                  <div key={s.id} style={{ marginBottom: 16 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
                      ...
                      <span style={{ color: c, fontSize: 12, fontWeight: 700 }}>{ss.total}/{ss.max} ({scoreLabel(ss.pct)})</span>
                    </div>
                    <div style={{ height: 6, background: "#1A1A1A", borderRadius: 3, overflow: "hidden" }}>
                      <div style={{ height: "100%", background: c, borderRadius: 3, width: `${ss.pct}%`, transition: "width 1.2s cubic-bezier(0.4,0,0.2,1)" }} />
                    </div>
                  </div>
                );
              })}
            ...
            {/* Strengths */}
            <div className="section-card" style={{ marginBottom: 16 }}>
              <p style={{ fontWeight: 700, fontSize: 11, color: "#22c55e", textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 14 }}>✅ Your Strengths</p>
              {segment.strengths.map((s, i) => (
                <div key={i} style={{ display: "flex", gap: 10, marginBottom: i < segment.strengths.length - 1 ? 12 : 0, alignItems: "flex-start" }}>
                  <div style={{ width: 20, height: 20, borderRadius: "50%", flexShrink: 0, marginTop: 1, background: "rgba(34,197,94,0.1)", border: "1px solid rgba(34,197,94,0.3)", display: "flex", alignItems: "center", justifyContent: "center", color: "#22c55e", fontSize: 10, fontWeight: 700 }}>✓</div>
            ...
            {/* Opportunities */}
            <div className="section-card" style={{ marginBottom: 16 }}>
              <p style={{ fontWeight: 700, fontSize: 11, color: "#FF6B35", textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 14 }}>🎯 Highest-ROI Opportunities</p>
              {segment.opportunities.map((o, i) => (
                <div key={i} style={{ display: "flex", gap: 10, marginBottom: i < segment.opportunities.length - 1 ? 12 : 0, alignItems: "flex-start" }}>
                  <div style={{ width: 20, height: 20, borderRadius: "50%", flexShrink: 0, marginTop: 1, background: "rgba(255,107,53,0.1)", border: "1px solid rgba(255,107,53,0.3)", display: "flex", alignItems: "center", justifyContent: "center", color: "#FF6B35", fontSize: 10, fontWeight: 800 }}>{i + 1}</div>
            ...
            <div className="section-card" style={{ borderColor: "rgba(0,217,255,0.2)", background: "rgba(0,217,255,0.04)", marginBottom: 16 }}>
              <p style={{ color: "#00D9FF", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 10 }}>💡 The Insight That Changes the Math</p>
            ...
            <div className="section-card" style={{ borderColor: "rgba(255,107,53,0.2)", background: "rgba(255,107,53,0.04)", marginBottom: 20 }}>
              <p style={{ color: "#FF6B35", fontSize: 11, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.8px", marginBottom: 10 }}>⏱ Why the Timing Matters</p>
            ...
            <div className="section-card" style={{ borderColor: "rgba(0,217,255,0.15)", background: "rgba(0,217,255,0.04)", marginBottom: 24 }}>
              <p style={{ fontSize: 13, color: "#999", lineHeight: 1.7 }}>
                📅 <strong style={{ color: "#fff" }}>It looks like you have an appointment set</strong> with our Senior Growth Consultant <strong style={{ color: "#00D9FF" }}>Rob</strong> at 11 AM to discuss next steps about the proposal that was sent over. We&apos;d love to be a part of your journey — and we&apos;d love to have you in the <strong style={{ color: "#00D9FF" }}>February Cohort</strong>.
              </p>
            </div>

            {/* CTAs */}
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              <a href="https://salestransformationgroup.com" target="_blank" rel="noopener noreferrer"
                style={{ display: "block", textAlign: "center", textDecoration: "none" }}
                className="cta-btn">
                Book My Strategy Call →
              </a>
              <a href="tel:8557848143"
                style={{ display: "block", textAlign: "center", textDecoration: "none" }}
            ...
            <div style={{ marginTop: 32, paddingTop: 20, borderTop: "1px solid #151515", textAlign: "center" }}>
              ...
              <p style={{ color: "#333", fontSize: 11, marginTop: 4 }}>1,000+ contractors transformed since 2018</p>
            </div>
          </motion.div>
        )}
